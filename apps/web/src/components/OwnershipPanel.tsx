  ...
  return (
    <div className="module-view">
      <div className="module-banner">
        <div>
          <div className="module-banner-title">Ownership &amp; Insider Activity</div>
          <div className="module-banner-sub">
            Section 16 insider transactions (Form 4) for the US-listed watchlist names, straight
            from SEC EDGAR.
          </div>
        </div>
      </div>
      ...
      <div className="note-banner">
        Only US-domestic issuers file Form 4. Foreign private issuers on the watchlist (SFL, South
        Bow) are exempt under Exchange Act Rule 3a12-3 and simply show no rows below.
      </div>
            ...
            {snapshot.transactions.map((t, i) => (
              <tr key={`${t.ticker}-${t.transactionDate}-${i}`}>
                ...
                <td className="short-name-cell">
                  {t.officerTitle ?? (t.isDirector ? "Director" : t.isOfficer ? "Officer" : "—")}
                </td>
                ...
                <td className="num-cell">{t.pricePerShare ? `$${t.pricePerShare.toFixed(2)}` : "—"}</td>
                <td className="num-cell">{t.sharesOwnedAfter.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
